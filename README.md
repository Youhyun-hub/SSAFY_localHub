...
🔗 **배포 사이트**: https://seoulocalhub.netlify.app/
...
| **관광정보**          | 7개 카테고리(관광지·문화시설·축제·쇼핑·숙박·레포츠·여행코스)를 구(區)별·검색어·(축제는 월별)로 필터링해 카드로 둘러보고, 같은 조건으로 카카오맵에서 위치까지 확인하는 페이지 |
...
같은 `.env` 파일에 **카카오맵 API 키**도 함께 등록하세요 (관광정보 페이지의 지도 시각화에 사용).

```
VITE_KAKAO_MAP_KEY=여기에-발급받은-카카오맵-키-입력
```

카카오 디벨로퍼스([developers.kakao.com](https://developers.kakao.com))에서 앱을 만들고, **앱 설정 → 플랫폼 → Web 플랫폼**에 아래 도메인을 반드시 등록해야 지도가 떠요 (키만 있고 도메인 등록이 없으면 실패해요):

```
http://localhost:5173
https://<배포도메인>.netlify.app
```
...
`dist/` 폴더가 생성돼요. Netlify에 올릴 때는 대시보드의 **Environment Variables**에도 `VITE_OPENAI_API_KEY`와 `VITE_KAKAO_MAP_KEY` **둘 다** 별도 등록해야 해요. 로컬 `.env`는 배포 서버에 자동으로 올라가지 않아요.
...
│   ├── PlaceCard.vue     장소 카드 (홈·관광정보 공용, 이미지 로드 실패 시 LocalHub 플레이스홀더로 대체)
│   └── KakaoMap.vue      관광정보 페이지 지도 시각화 (좌표 이상치 자동 필터링 포함)
│
...
- **지도 시각화** (Kakao Maps, 관광정보 페이지 내 필터 연동 + 좌표 이상치 자동 제외)
- 축제 월별 필터 (`eventstartdate` 기준)
...
- **외부 API**: OpenAI Chat Completions API (챗봇), Open-Meteo (날씨, 키 불필요), Kakao Maps JavaScript SDK (지도 시각화, 도메인 등록 필요)
- **배포**: Netlify
...
- 지도 데이터: Kakao Maps JavaScript SDK (카카오 디벨로퍼스 이용약관 준수, 앱 키 발급 및 사용 도메인 등록 필요) — https://developers.kakao.com

> ⚠️ TourAPI 원본 데이터 중 일부 항목(서울 관광지 783건 중 8건)은 좌표 필드(`mapx`/`mapy`)가 원본 자체에서 잘못 수집되어 있어(한국 영토 범위를 크게 벗어난 값), 지도 시각화 시 자동으로 제외 처리됩니다. 카드 목록에는 정상 노출되며, 지도 마커에서만 빠집니다.
...
**지도가 안 뜨거나 "카카오 지도 API 키를 등록해주세요"라고만 나와요**
→ `VITE_KAKAO_MAP_KEY`가 `.env`(로컬) 또는 Netlify 환경변수(배포)에 등록됐는지 확인하세요.

**지도는 뜨는데 서울이 아니라 엉뚱하게 축소되어 보여요 (중국·일본 쪽까지 보임)**
→ 카카오 디벨로퍼스에 **현재 접속 중인 도메인**(`localhost:5173` 또는 실제 배포 도메인)이 Web 플랫폼으로 등록되어 있는지 확인하세요. 등록되어 있는데도 재현되면, `KakaoMap.vue`의 좌표 이상치 필터링 범위(위도 33~39, 경도 124~132)를 벗어나는 새로운 이상치 데이터가 있는 건 아닌지 확인해보세요.
